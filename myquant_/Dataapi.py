...
class DataApi(object):
    ...
    def __init__(self, price='close', fq='post',
        ...
        try:
            ...
            print('jqdata import FAILED')
        except ImportError:
            import jqdatasdk
            self._api = jqdatasdk
            self._api_name = 'jqdatasdk'
            print('jqdata import SUCCEEDED')
    ...
    def get_prices(self, securities, start_date=None, end_date=None,fields=None,
                   count=None):
        return self._get_price(
            fields=fields, securities=securities,
    ...
    def get_factors(self,securities,factors,start_date=None,end_date=None):

        # if (self._api_name == 'jqdata'):
        #     print('jqdatasdk not IMPORT')
        #     get_factor = partial(self.api.get_price,
        #                         pre_factor_ref_date=end_date)
        # else:
        #     print('jqdatasdk imported')
        get_factor = self.api.get_factor_values
    ...
    def is_predefined_factors(self,factors):
        if len(factors):

            factor_list = self.api.get_all_factors()
            factors_vaild = [factor for factor in factors if
                             (factor in factor_list['factor'].values)
                             ]

            # print(factors_vaild)

            if len(factors_vaild) < len(factors):
                print("Factors name [{}] not valid, ignore.".format(set.difference(set(factors), set(factors_vaild))))
            return factors_vaild
